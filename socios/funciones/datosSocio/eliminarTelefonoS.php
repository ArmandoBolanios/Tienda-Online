...
require_once '../../connections/conexion.php';    
   $sesionSocio = $_SESSION['userSession']; 
	...
	/*Inicia validacion del lado del servidor*/
	 if (empty($_POST['id_telefono'])){
			$errors[] = "ID vacío";
		}   else if (
			!empty($_POST['id_telefono']) ){
 
		// escaping, additionally removing everything that could be (html/javascript-) code
		$id_tel=intval($_POST['id_telefono']);
		
		$sql="DELETE FROM socio_telefono WHERE idSocioTelefono='".$id_tel."'";
		$query_deleteT = mysqli_query($conexionBD,$sql);
			if ($query_deleteT){
